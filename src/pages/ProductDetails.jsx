...
import CustomCarouselProducts from "@/components/layouts/CustomCarouselProducts";
import {
    Accordion,
    AccordionBody,
    AccordionHeader,
                    ...
                    <ul className="border border-gray-700 rounded-lg p-4 grid gap-3">
                        {reservstion.map((item) => (
                            <li key={item.id} className="list-none flex">
                                ...
                                <Typography as={"h4"} className="lg:text-base text-xs ml-6">
                                    {item.text}
                                </Typography>
                            </li>
                        ))}
            ...
            <div className="px-6 mt-2">
                <div>
                    ...
                    <Accordion key={item.id} open={open === item.id} icon={<Icon id={item.id} open={open} />}>
                        <AccordionHeader className='text-lg' onClick={() => handleOpen(item.id)}>{item.title}</AccordionHeader>
                        <AccordionBody>
